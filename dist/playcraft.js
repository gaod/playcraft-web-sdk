...
const getHlsQualityOptions = async source => {
  ...
  const manifest = (await axios__default["default"].get(selected.src)).data;
...
const PremiumPlayer = ({
  ...
  const preloadQualityOptions = React.useRef();
  React.useEffect(() => {
    if (source.length > 0) {
  ...
  const handlePlaybackStateChange = (event, state) => {
    ...
    if (lastState.current === 'loading' && Math.abs(targetState.currentTime - videoRef.current.currentTime) > 5) {
      return;
    }
