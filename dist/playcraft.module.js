  ...
  const getMessage = (code, property) => {
    return (
      ...
      translations[code].replace(/{(\S+?)}/gi, (substring, name) => {
        ...
        const res =
          (_get2 = _get(property, name)) !== null && _get2 !== void 0
            ? _get2
            : substring
...
const useSettings = () => {
  ...
  const {mediaSources} = useContext(Context.UI).state
...
const useChangeSettings = () => {
  const {options, dispatch} = useContext(Context.Module)
  const player = usePlayer()
...
const useMenu = () => {
  const {dispatch} = useContext(Context.Adapter)
...
const AutoplayDialog = () => {
  ...
  const {dispatch} = useContext(Context.API)
  useEffect(() => {
    if (open) {
      dispatch(
...
const loadOverrides = async ({hls, filterQualities}) => {
  ...
  const {data} = await axios.get(hls)
...
const PlayerUi = ({options, coverImageUrl, videoRef, playerRef}) => {
  ...
  const {dispatch} = useContext(Context.Adapter)
  const [widevineLevel, setWidevineLevel] = useState('loading')
  useEffect(() => {
...
export {
  CastVideoButton as CastButton,
  CastConsumer,
  CastSender,
  MediaTailorPlugin,
  EnterprisePlayer as Player,
  SupportEnvironment,
  getDefaultDrmConfig,
