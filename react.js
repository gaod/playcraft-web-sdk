...
const subscribePlaybackState = (media, updateState) => {
  ...
  const registered = [on(media, 'error', event => updateIfChanged(event, 'error')), on(media, 'waiting', updateBufferingState), on(media, 'loadstart', event => updateIfChanged(event, 'loading')), on(media, 'loadeddata', event => setTimeout(() => {
